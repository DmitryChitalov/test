...
class Item(object):
    ...
    def __init__(self, tab_id, req_id, region, text_request, status,
                 vac_number, created):
        self.tab_id = tab_id
    ...
    def get_elements():
        ...
        rows = db_session.query(Request).filter(
            Request.user_id == session["user_id"]).order_by(
        ...
        for row_request in rows:
            ...
            items_for_table.append(
                Item(id_n, row_request.id, row_request.region,
                     row_request.text_request, status, vac_numbers,
                     row_request.created))
            id_n += 1
...
def single_item(id):
    ...
    if session["user_id"]:
        ...
        row: Request = db_session.query(Request).filter(
            Request.id == element.request_id).one()
        if row.file_name is not None:
            ...
            for key, value in salary_average.items():
                sum_salaries.append(key + "   от: " + '{:6.0f}'.format(
                    value[0]) + "₽.  до: " + '{:6.0f}'.format(value[1]) + "₽.")
